...
For example, they can be used to decide between multiple choices and to selectively direct attention.
...
Winner-take-all (WTA) networks are mechanisms that select the largest value among a number of possible inputs.
...
This mechanism is regularly employed as a component of cognitive models involving selective attention~\cite<e.g.,>{itti1998,standage2005} and decision making -- in which the action with the highest utility is selected to drive behaviour~\cite<e.g.,>{oreilly1998}.
...
To test and compare the two WTA mechanisms we provide an input of $\rho_i = u - s(1 - \delta_{1i}) + \eta_i$, where $u$ is the strength of the largest input, $s > 0$ is the target separation relative to all other inputs, $\delta$ is the Kronecker delta, and $\eta_i$ is Gaussian (white) noise with standard deviation $\sigma$.
...
It is important to note that $u$ does not only determine the size of the largest input, but also the general baseline of inputs.
...
This does not take into account whether the winning output actually corresponds to the largest input, but for some models it is more desirable to produce a clear incorrect decision than an unstable incorrect decision.
...
In addition, the IA network is easily extendible to dynamically control the speed of decisions, by supplying an external bias to layer 2 to adjust the $\vartheta$ threshold.
...
We did not investigate the agreement of these mechanisms with neurobiological and behavioural data because this has been done before for different WTA networks~\cite{gold2007,smith2004} and is outside the scope of this paper.
...
The implementation in spiking neurons, however, ensures a certain level of biological plausibility and more readily permits comparisons with neural data.
...
Nevertheless, the qualitative results stay the same.
...
\todo{End on a more finalizing note. We should try to make it clear how in the end this has advanced the field. Why is this work important, where does this leave us, etc.}
