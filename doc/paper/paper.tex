...
\title{A Spiking Independent Accumulator Model for Winner-Take-All Networks
\todo{I put 'networks', but am not sure that's the right word, but it seems some word
needs to go there... 'computations', 'decisions'? WTA is usually an adjective}}
...
In particular, the LCA networks predicts firing rates proportional to the input strength, while the IA network predicts that firing rates will rise up to a threshold for neurons in the first neurons and that neurons in the second layer will not become active before reaching this threshold.
With regard to behavioural data, different effects for decision times are predicted as the number of choices increases:
for the LCA network decisions get slower due to the mutual interaction, but for the IA network decisions will take less time because only a single accumulator needs to exceed the threshold.
Nevertheless, given specific evidence for one network does not exclude the possibility that the other network is employed for different tasks or in different brain areas.
