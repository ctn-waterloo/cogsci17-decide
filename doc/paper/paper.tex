...
We provide a novel spiking implementation of the well-known leaky, competing accumulator (LCA) model, by mapping the dynamics onto a population-level representation.
...
The winner-take-all (WTA) mechanism is a component that selects the largest value among a number of possible inputs.
More specifically, the input is a $d$-dimensional vector corresponding to the non-negative utility of $d$ different choices, and the desired output is positive for the dimension with highest utility (i.e., the ``winner'') and zero for all others.
This mechanism is regularly employed in cognitive models of decision making, which must select the most important action to take at any given moment in time~\cite<e.g.,>{oreilly1998}. 
...
In particular, we map each mechanism onto a biologically plausible network of spiking neurons, and then compare them using a set of benchmarks that are normative in nature.
The first mechanism is an implementation of the {\it leaky, competing accumulator} (LCA) model by~\citeA{usher2001}.
It has been widely used, for example in versions of the Temporal Context Model~\cite{sederberg2008}, and in some of our models~\cite<e.g.,>{kajic2017}.
\todo{Add sentence somewhere differentiating our implementation from the latter implementation}
The second mechanism is our proposed {\it independent accumulator} (IA) model, which involves a second thresholding layer that is recurrently connected to a primary integrating layer.
For both mechanisms we are interested in decision-making tasks involving more than two choices ($d > 2$).
\todo{Misplaced sentence / unimportant here}

To implement each model, we use the Neural Engineering Framework~\cite<NEF;>{eliasmith2003} to map their dynamics onto populations of spiking neurons.
...
We provide a short introduction to the NEF, describe our implementation of the two WTA mechanisms, present our benchmarks, and finally discuss some resulting implications for cognitive modelling.
...
We now summarize the relevant aspects of this framework, ordered by its three principles: \emph{representation}, \emph{transformation}, and \emph{dynamics}.
...
\end{equation}
where $\alpha_i$ is a gain factor, $J_i^{\mathrm{bias}}$ a bias current, and $G_i \left[ \cdot \right]$ is the neural nonlinearity.
Here, we use spiking, leaky integrate-and-fire (LIF) neurons for $G_i \left[ \cdot \right]$.
...
% should we mention that we use Principle 3 to implement each integrator
...
% ^ this isn't quit equivalent. you also need to multiply the Heaviside by lambda in order to keep the feedback having the same relative magnitude
