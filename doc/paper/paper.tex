...
For example, it can be used to decide among multiple choices or direct attention to important parts of a visual scene.
...
This mechanism is regularly employed as a component of cognitive models involving decision making, in which the action with the highest utility is selected to drive behaviour~\cite<e.g.,>{itti1998,standage2005}.
...
The dynamics for each state variable $x_i(t),\ 1 \leq i \leq D$, where $D$ is the number of choices, are given by~(see Fig.~\ref{fig:usher-mcclelland}a):
...
The other WTA mechanism that we investigate is our proposed independent accumulator (IA) model (Figure~\ref{fig:ia-sketch}).
...
\begin{equation}
    ...
    \end{split}
\end{equation}
Compared to the LCA model, the threshold $\vartheta$ is an additional free parameter that controls how quickly the IA model makes a decision and how much evidence will be integrated.
Instead of directly manipulating $\vartheta$, we opt to change $\tau_1$ instead which is equivalent due to the leak-free integration.
...
This allows us to figure out which functions are performed more robustly by each network.
...
Thus, we define $E_{\max}$ as the `highest output of a losing choice' during the whole simulation.
...
Also, the baseline has an important influence.
For a small baseline of $u=0.2$ the winner will mostly not exceed the 0.15 threshold with noise present.
The best performance is achieved with a medium baseline of $u=0.6$.
A higher baseline makes it more likely that runner-ups will exceed the 0.15 threshold, especially with a small target separation.
...
\begin{figure*}
    ...
    \caption{
        Fraction of trials with a clear decision for different input noise standard deviations $\sigma$, target separations $s$, and input baselines $u$ for the LCA network.
...
While always reaching a decision, the decisions of the IA network are not always correct (see Fig.~\ref{fig:correct}).
Overall, the IA performance tends to be worse for a high baseline, but better for a low baseline (Fig.~\ref{fig:correct}a,b).
We can greatly improve the IA performance by increasing $\tau_1$ to \SI{0.5}{\second} which effectively adjusts the threshold $\vartheta$ (Fig.~\ref{fig:correct}c).
...
For a low baseline of $u=0.2$ this makes the IA network unable to decide within the allocated time frame, but given more time it would still reach a decision.
...
The time required to reach a decision in the LCA network depends mostly on the baseline and target separation (see Fig.~\ref{fig:time}a).
The level of noise only had a minor influence and was averaged over.
Similarly, in the IA network the baseline is also the most important factor (dashed vs. solid lines in Fig.~\ref{fig:time}b).
...
\begin{figure*}
    ...
    \caption{
        (a) Mean decision times for different baselines $u$ and target separations $s$ for the LCA.
        (b) For the IA network $u = 0.2$ (solid) and $u = 1.0$ (dashed) are shown for different noise standard deviations $s$ and target separations $s$.
        Error bars denote bootstrapped 95\% confidence intervals.
    }\label{fig:time}
\end{figure*}
...
The magnitude of such transient responses is reduced by decreasing by adjusting $\tau_1$ to 0.5.
\begin{figure*}
    \centering
...
For instance, the LCA network can determine the correct winner more quickly and given a conclusive decision was made, it is always picks the correct winner.
...
Both models might produce a transient response that can be problematic in detecting when the decision has been made as it might be interpreted as the decision.
This is of special relevance when integrating the networks in larger cognitive models.
...
We did not investigate the biological plausibility of these mechanisms in detail because this has been done before for different WTA networks~\cite{gold2007,smith2004}.
