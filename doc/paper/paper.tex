...
A higher baseline makes it more likely that runner-ups will exceed the 0.15 threshold, especially wit ha small target separation.
With a small baseline, on the other hand, even the winner can be unable to exceed the threshold in the presence of noise.
...
Interestingly, in 99.97\% of successful decisions, the correct winner was determined by the LCA network.
...
Overall, the IA performance tends to worse for a high baseline, but better for a low baseline.
We can greatly improve the IA performance by increasing $\tau_1$ which effectively adjusts the threshold $\vartheta$.
This will, however, make the network slower to decide.
A further improvement is possible by eliminating a systematic bias introduced in the second layer of the IA network.
...
By using identical layer 2 populations, i.e.~identical gain and bias parameters for each population, this bias can be eliminated and the IA network performs even better.
...
The time required to reach a decision in the LCA network depends mostly on the baseline and target separation (see Fig.~\ref{fig:time}\todo{put a and b here too and reference them in the discussion}).
Similarly, in the IA network the magnitude of the largest input is also the most important factor.
...
\todo{this might need some rewriting given new results}
\begin{figure*}
    \centering
...
For instance, the LCA network determines the correct winner more quickly and given a conclusive decision was made it is almost always the correct decision.
...
The IA network might not be as quick or find the correct winner as often depending on the choice of $\tau_1$, but given enough time it will eventually arrive at a decision and produce a clear output.
