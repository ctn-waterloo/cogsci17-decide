...
\begin{figure*}[t]
    \centering
    \includegraphics[scale=0.9]{figures/decisions}
    \caption{
...
\begin{figure*}[t]
    \centering
    \includegraphics[scale=0.9]{figures/correct}
    \caption{
...
\begin{figure*}
    \centering
    \includegraphics[scale=0.9]{figures/transient}
    \caption{
...
As an example, we intend to use the IA network in a free recall memory model where a clear selection of successive responses needs to be made, but correctly identifying the single item remembered most strongly is not required.
...
However, the evidence integration uses competition by means of pooled inhibition makes it more similar to the LCA than the IA network and the behaviour under highly noisy conditions is unknown.
...
Neurons in the macaque lateral intraparietal area give a similar step response~\cite{latimer2015}.
