...
\begin{equation}
    ...
    \end{split}
\end{equation}
Notably, this takes on the form of Equation~\ref{eqn:usher-mcclelland} after substituting $\tau = \tau_1$, $k = -\tau_1/\tau_2$, and $\beta = \bar{\beta}\tau_1/\tau_2$, with the only remaining difference being the Heaviside nonlinearity applied to the state feedback.
In contrast to the LCA model, the Heaviside threshold $\vartheta$ is a free parameter that controls how much evidence needs to be integrated before enabling any competition between states.
...
\begin{figure*}[t]
    ...
    \caption{
        ...
        \todo{Move (dashed) and (solid) from title to legend?)}
        Error bars denote bootstrapped 95\% confidence intervals.
    }\label{fig:time}
\end{figure*}
...
This can be problematic for cognitive models that require the network to make a decision (even when it may be incorrect).
...
On the other hand, the LCA network is especially well-suited for situations where a decision needs to be continuously adjusted.
...
\todo{End on a more finalizing note.}
