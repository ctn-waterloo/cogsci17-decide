...
data~\cite<e.g.,>{bogacz2006,gold2007,smith2004}.  Here, we 
investigate the suitability of two different WTA mechanisms in the context of 
large-scale cognitive modelling.
...
are normative in nature.  The first mechanism is an implementation of the 
...
Model~\cite{sederberg2008}, and some of our models~\cite<e.g.,>{kajic2017}. \todo{How does the latter implementation differ from the implementation here?}
...
To implement these models, we use the Neural Engineering 
Framework~\cite<NEF;>{eliasmith2003} to map some prescribed dynamics onto populations of spiking neurons. The NEF has been used for a wide variety 
of models, including the largest functioning simulation of the human brain~\cite{eliasmith2012}.  We first provide a short introduction to the NEF, and then describe the implementation of the two WTA mechanisms.
...
\end{equation}
where $h(t) = \tau_{\mathrm{s}}^{-1}\exp(t/\tau_{\mathrm{s}})$ is an exponential 
...
FA8655-13-1-3084, CFI, OIT, and NSERC CGS-D\@.  % chktex 8
